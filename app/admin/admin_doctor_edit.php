...
$selectSql = 'SELECT doctor_id, fullname, specialization, email FROM Doctor WHERE doctor_id = ? LIMIT 1';
...
$doctor = mysqli_fetch_assoc($selectResult);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $checkSql = 'SELECT doctor_id FROM Doctor WHERE email = ? AND doctor_id <> ? LIMIT 1';
    ...
    if (empty($errors)) {
        $updateSql = 'UPDATE Doctor SET fullname = ?, specialization = ?, email = ? WHERE doctor_id = ?';
        $updateStmt = mysqli_prepare($conn, $updateSql);

        if ($updateStmt === false) {
            $errors[] = 'Unable to update doctor right now.';
        } else {
            mysqli_stmt_bind_param($updateStmt, 'sssi', $fullname, $specialization, $email, $id);

            if (mysqli_stmt_execute($updateStmt)) {
                mysqli_stmt_close($updateStmt);
                header('Location: admin_doctors.php?success=updated');
                exit;
            }

            mysqli_stmt_close($updateStmt);
            $errors[] = 'Failed to update doctor. Please try again.';
        }
    }
}
?>
...
<body>
    <div class="page-wrap">
        <div class="panel">
            <h2 class="mb-3">Admin - Edit Doctor</h2>
